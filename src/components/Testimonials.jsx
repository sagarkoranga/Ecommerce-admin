...
export default function Testimonials() {
  ...
  const toggleStatus = async (id, status) => {
    await axios.patch(`http://localhost:3000/testimonials/${id}/status`, { status });
    load();
  };
  ...
  const save = async () => {
    ...
    if (editId) {
      await axios.patch(`http://localhost:3000/testimonials/${editId}`, fd);
    } else {
      await axios.post("http://localhost:3000/testimonials", fd);
    }
